...
| DEF-002 | High | AC-FR01-01 | 1. `tests/boundary/test_ac_fr01_01_null_grid.py` 기대값 확인<br>2. PRD §13.1 / Report/02 Error Code 표 대조 | `code="ERR_NULL_GRID"`, `message="Input grid is null."` | 테스트는 `code="INVALID_SIZE"`, `message="Grid must be 4x4."` 기대 | RED 테스트가 PRD·Report/02 SSOT와 다른 Error code/message 사용 | 테스트 또는 구현 중 **한쪽을 SSOT로 통일** (권장: PRD `ERR_NULL_GRID`로 테스트 정렬) |
| DEF-003 | High | AC-FR01-01 | 1. `test_ac_fr01_01_null_grid.py`의 `@patch` 대상 확인<br>2. `.cursor/rules/magicsquare-ecb-architecture.mdc` Domain 진입점 확인 | Domain 격리 대상 = `solve_partial_grid` (Control) | `@patch("magicsquare.boundary.entry.resolve")` — 프로젝트에 `resolve` 미정의 | 테스트가 아키텍처 문서와 다른 함수명으로 mock | patch 경로를 `solve_partial_grid`(또는 Boundary가 주입받는 port)로 변경; README TC-A-04와 일치 |
